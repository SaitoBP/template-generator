...
const valueOrDefault = (
	...
	if (object) {
		if ((property in object && object[property] === undefined) || !(property in object)) {
...
function Commarize(value: number | string, options: CurrencyOptions & QuantityOptions) {
...
export const useFormatNumber = (settings?: CurrencyOptions & QuantityOptions) => {
	const obj: {
		money: MoneyFn;
		config: ConfigFn;
		format: FormatFn;
		...
		money: (number, options) => {
			...
			try {
				...
				if (op.trunc) number = trunc(number as number, op.digits || 0);
				...
				if (op.debug) {
					console.warn(`formatting number: ${number} cause an exception "${e.message}"`);
				}
				return number;
			}
		},
		config: (options) => {
			...
			return {
				...moneySettings,
				digits: options?.digits || settings?.digits || moneySettings?.digits || 0
			};
		},
		clamp: (value, min, max) => {
		...
		quantity: (number, options) => {
			...
			try {
				...
				} else if (op.trunc) number = trunc(number as number, op.digits || 0);
